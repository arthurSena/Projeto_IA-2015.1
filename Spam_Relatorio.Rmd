...
Agora vamos realizar nossas predições com os novos modelos.
```{r  message=FALSE, warning=FALSE}
teste[,"spam"]<- as.factor(teste$spam)
#KNN
pred_knn<-predict(knn_model, newdata = as.data.frame(teste),na.action=na.omit)
...
#Treinando com emails que foram escritos usando HTML - Terceiro Modelo
Nesse cenário, vamos utilizar a variável "format" a fim de predizer nossos e-mails. Tal variável afirma se um e-mail foi escrito usando apenas HTML ou não. Temos o palpite que e-mails que foram escritos em HTML apresentam uma boa probabilidade de serem considerados spam's.
...
# control = trainControl(method = "repeatedcv", number=10,repeats = 10)
# knn_grid = expand.grid(k=c(1:20))
# knn_model = train(spam~format,  data=treino ,method="knn",preProcess=c("range"),tuneGrid = knn_grid,trControl=control, na.action=na.omit)
# 
# #Usando o SVM
# control <- trainControl(method="repeatedcv", number=10,repeats=10)
# modelSvm <- train(spam~format, data=treino, method="svmRadial", trControl=control)
# 
# #Usando C50
# control <- trainControl(method="repeatedcv", number=10, repeats=10)
# modelC50<- train(spam~format, data=treino, method="C5.0", trControl=control)
```
...
```{r}
knn_import <- varImp(knn_model, scale = FALSE)
knn_import

svm_import <- varImp(modelSvm, scale = FALSE)
svm_import

c50_import <- varImp(modelC50, scale = FALSE)
c50_import

```

Agora, vamos refazer nossos modelos usando as variáveis com maior influência na classificação dos modelos.
...
control = trainControl(method = "repeatedcv", number=10,repeats = 10)
knn_grid = expand.grid(k=c(1:20))
knn_model = train(spam~num_char+format+line_breaks+number,  data=treino ,method="knn",preProcess=c("range"),tuneGrid = knn_grid,trControl=control, na.action=na.omit)

#Usando o SVM
control <- trainControl(method="repeatedcv", number=10,repeats=10)
modelSvm <- train(spam~num_char+format+line_breaks+number+time+exclaim_mess, data=treino, method="svmRadial", trControl=control)

#Usando C50
control <- trainControl(method="repeatedcv", number=10, repeats=10)
modelC50<- train(spam~num_char+format+line_breaks+number+time+exclaim_mess, data=treino, method="C5.0", trControl=control)
```
